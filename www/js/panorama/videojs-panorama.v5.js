...
var textInfoTitle = [
    ["Đại Dương Vincity Ocean Park"],
    ["Biển hồ nước mặn" ],
    ["Hồ lớn trung tâm" ],
    ["Vincity Ocean Park"],
    ["Công viên ngoài trời"],
    ["Công viên BBQ"],
    ["Đại học quốc tế VinUni"],
    ["Khuôn viên Đại học VinUni sẽ được chia thành 3 công trình lớn:"],
...
var textInfoP = [
    ["Bạn đang ở thành phố Đại Dương Vincity Ocean Park với thiên đường biển ngay trong lòng Hà Nội."],
    ["Biển hồ nước mặn 6,1ha với màu nước lam ngọc, bãi cát trắng cùng những rặng dừa xanh mát, hệt như biển Maldives hay Bali."],
    ["24,5ha hồ lớn trung tâm với các bãi cát trắng ven hồ"],
    ["Vincity Ocean Park, nơi bạn thoải mái sống mới muôn vàn tiện ích."],
    ["700 máy tập đa dạng được phân bố tập trung tại công viên thể thao ngoài trời và xen kẽ tại các công viên nội khu."],
    ["6 công viên BBQ với 100 điểm nướng dã ngoại cho cả nhà."],
    ["Nơi không chỉ có niềm vui hạnh phúc mà còn ghi dấu những thành công."],
    ["Tòa nhà chính có chiều cao 10 tầng với tháp cao 108 m lấy ý tưởng từ hình tượng “đôi cánh tri thức”.<br> Khu Ký túc xá hiện đại có tổng diện tích xây dựng gần 10.600 m2, có đầy đủ căng tin, khu tự học, phòng y tế, các cửa hàng tiện ích và khu nghỉ ngơi, tạo nên môi trường sống và học tập chất lượng cho sinh viên. <br> Khu Liên hợp Thể thao có Nhà thể thao (diện tích 4400 m2), khu vực sân vận động đa chức năng và các khu thể thao ngoài trời được thiết kế theo tiêu chuẩn Olympic."],
...
var BaseCanvas = function BaseCanvas(baseComponent, THREE) {
    ...
    return {
        ...
        handleMouseDown: function handleMouseDown(event) {
            ...
            //var intersects = this.raycaster.intersectObjects(this.scene.children, !0);
                if (intersects.length > 0 ) {
                    console.log(intersects[0].point);
                    console.log(intersects[0].object.name);
                    localStorage.setItem("link",intersects[0].object.name);
                    this.player().pause();
                    $(".player_wrapper").fadeOut(1500);
                    
                    $("#infoHotSpot").fadeIn(1500);
                    if(localStorage.getItem("link") !== null){
                        $("#imgInfo").attr("src","img/Vincity/"+localStorage.getItem("link")+".jpg");
                        $("#Info_title").html(textInfoTitle[localStorage.getItem("link")]);
                        $("#Info_p").html(textInfoP[localStorage.getItem("link")]);
                     }
                }
        },
        ...
        render: function render() {
            ...
            if(this.mouseDown === false && !this.player().paused()){
                this.lon += 0.009; 
                this.lon = Math.max(this.settings.minLon, Math.min(this.settings.maxLon, this.lon));
            }
