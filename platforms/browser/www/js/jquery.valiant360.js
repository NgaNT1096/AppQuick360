...
(function ($, THREE, Detector, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        init: function () {
            ...
            console.log(localStorage.getItem("poitLon"));
            
            this._lat = this.options.lat;
            this._lon = this.options.lon;
            
            if(localStorage.getItem("poitLat") !== null && localStorage.getItem("poitLon") !== null){
                this._lat = localStorage.getItem("poitLat");
                this._lon = localStorage.getItem("poitLon");
        ...
        createMediaPlayer: function () {
            ...
            // adds camera to array
            scenes[pluginName] = this._scene;
            cameras[pluginName] = this._camera;
            renderers[pluginName] = this._renderer;
            poitLat[pluginName] =  this._lon;
                        ...
                        $(document).keydown(function (e) {
                            if (e.which == 32) {
                                if (!video.ended) {
                                    if (!video.paused) {
                                        video.pause();
                                        $("#playImg").attr("src","img/play.png"); $(".state").slideToggle().delay(1500).slideToggle().text("Video paused. Please click again to record time and position.");
                                    } else {
                                        video.play();
            ...
            this._mesh = new THREE.Mesh(new THREE.SphereGeometry(500, 80, 50), new THREE.MeshBasicMaterial({
                map: this._texture
            }));
        ...
        createControls: function () {
            ...
            //  var fullScreenControl = this.options.autoplay ? 'fa-pause' : 'fa-play';

            var controlsHTML = ' \
                <div class="controls"> \
                    <div id="slide" class="">\
                    <span class="slideControlContainer"><span class="slideControlFill" style="width:10%"><span class="slideControlHandle"></span></span></span>\
                </div> \
                    <ul id="btn">\
                        <li id="play"><a href="#" class="playButton button fa ' + playPauseControl + '"><img id="playImg" src="img/pause.png" style="width: 50px;"></a></li> \
                        <li id="time"><span class=""><span class=""></span></span></li>\
                        <li id="volum"><a href="#" class="muteButton button fa ' + muteControl + '"><img src="img/volume-up.png" style="width: 50px;"></a></li> \
                        <li id="fullscreen"><a href="#" class="fullscreenButton button fa "><img src="img/fullscreen.png" style="width: 50px;"></a></li> \
                    </ul>\
                </div> \
            ';
        ...
        saveLocalCamera: function(){
                if (typeof(Storage) !== "undefined") {

                    localStorage.setItem("poitLat",this._lat);
                    localStorage.setItem("poitLon",this._lon);
        ...
        update: function() {
            ...
            this._lon = Math.max(-360, Math.min(360, this._lon));
            if(this._lon === 360){ 
                this._lon = -360;
            }else if(this._lon === -360){
                this._lon = 360;
            }
            ...
            // updates position for debug sphere
            if (this.options.debug) {
                var selectedObject = this._scene.getObjectByName('debugSphere');
                selectedObject.position.set(cx, cy, cz);
            }
        },
